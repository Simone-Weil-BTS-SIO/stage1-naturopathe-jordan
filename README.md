## Pour initialiser le projet vous allez avoir besoin d'une config.

> Pour cela utilisez ce template. Créez le fichier prod.ini et mettez le dans le dossier app/Config/
...
## Vous aurez aussi besoin d'une base de donnée.

> Pour la mettre en place il faut y intégrer ces deux scripts.
...
GRANT ALL ON naturopathie.* TO '{login}' @'localhost' IDENTIFIED BY '{mot_de_passe}e' WITH GRANT OPTION;
FLUSH PRIVILEGES;
```

# :warning: **Pensez à remplacer:**

- **{login} par l'identifiant du compte mysql**
  
- **{mot_de_passe} par le mot de passe du compte mysql**
  
- **{database} par le nom de la base de donnée**
